...
- 把会话原生导入到目标工具的本地历史格式。
- 把会话导出为可携带的 JSON 捆绑包，在另一台机器（比如 SSH 远程机）上导入。
...
捆绑包包含会话元数据（来源、标题、项目路径、时间戳）以及消息和工件。写入前会先脱敏并把嵌入的 base64 图片替换成占位说明，所以这个文件可以放心在机器之间传输。在 TUI 里，每个来源的目标选择界面都有 `导出捆绑包 (.json)` 选项。
